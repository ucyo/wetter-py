...
DOCKER_VERSION_NUMBER="${DOCKER_USERNAME}/wetter:${DOCKER_TAG}"
DOCKER_FILE="${SOURCE_DIR}/../"
DOCKER_VERSION_RELATIVE="${DOCKER_USERNAME}/wetter:testing"

if [[ $DOCKER_TAG != *"rc"* ]]; then
  DOCKER_VERSION_RELATIVE="${DOCKER_USERNAME}/wetter:latest"
fi
...
docker push "ucyo/wetter" --all-tags --quiet
